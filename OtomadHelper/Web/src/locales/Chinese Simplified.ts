...
export default {
	javascript: {
		...
		descriptions: {
			...
			sonar: {
				_: "声呐是利用区域裁切的形状创建节拍风格动态图形的视觉效果。本功能可利用乐谱中的鼓组乐器（通道10）为其各种打击乐器分配不同的形状或效果。\n一种常见的音MAD流派是根据节奏在画面中加入例如扩散的圆圈等各种图形来制作动态图形（MG动画）。MG动画是介于平面设计与动画设计之间的一种产物，是基于时间流动而设计的视觉表现形式，是影像艺术的一种。其中“动态图形”指的是会随时间流动而改变形态的图形。仅包含MG动画的单个音MAD视频通常以《图形》《形状》等视频标题命名。该效果的名称“声呐”由许祭命名。",
			},
			shupelunker: {
