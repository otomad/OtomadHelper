	...
	javascript: {
		...
		descriptions: {
			...
			source: {
				...
				randomOffsetForTracks: "开启后，这可能会导致随机选择的素材片段具有不同的原始音高，因此只能用来做成搞笑视频供娱乐，几乎不能做成高品质视频",
			},
			score: {
				trim: "截取乐曲生成的时间范围",
				bpm: "指定每分钟多少拍",
			...
			stream: {
				...
				tuning: {
					tuningMethod: {
						noTuning: "无变调效果",
						pitchShift: "使用移调音频效果插件，它很慢，但可以超出音域，并且是唯一一个可以在VEGAS Pro ≤ 15中访问脚本API的方法",
			...
			prve: {
				control: {
					general: "一般条件下控制不同音高和相同音节的事件（音MAD和人力模式）。\n如果其它分别控制已关闭，它们的用例也将被包含在这里。",
					samePitch: "对相同音高的事件分别控制（音MAD和人力模式）",
					differentSyllables: "对不同音节的事件分别控制（人力模式）",
				},
				whirl: "爱的魔力转圈圈 = 乒乓效应 + 水平翻转",
			},
			pixelScaling: {
